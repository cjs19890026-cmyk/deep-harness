...
const translations = {
  en: {
    ...
    'settings.check.cliOk': '✓ Obsidian CLI: {path}',
    'settings.check.cliMissing': '✗ Obsidian CLI not found — install Obsidian 1.12+ and enable "Command line interface" (the agent falls back to file tools).',
    ...
    'settings.obsidianSkill.desc': 'Ship the "obsidian" skill (vault conventions + official CLI reference) into the agent, so vault tasks use Obsidian-aware operations.',
    'settings.obsidianCli.name': 'Obsidian CLI path',
    'settings.obsidianCli.desc': 'Official Obsidian CLI (Obsidian 1.12+, enabled in Settings → General → Command line interface). Leave empty to auto-detect.',
    'settings.obsidianCli.placeholder': '/usr/local/bin/obsidian',
  ...
  zh: {
    ...
    'settings.check.cliOk': '✓ Obsidian CLI:{path}',
    'settings.check.cliMissing': '✗ 未找到 Obsidian CLI——需 Obsidian 1.12+ 并在「设置→通用→命令行界面」中开启(未开启时 agent 会降级为文件工具)。',
    ...
    'settings.obsidianSkill.desc': '把 obsidian 技能(vault 约定 + 官方 CLI 速查)注入 agent,让 vault 任务用 Obsidian 语义操作。',
    'settings.obsidianCli.name': 'Obsidian CLI 路径',
    'settings.obsidianCli.desc': '官方 Obsidian CLI(需 Obsidian 1.12+,并在 设置→通用→命令行界面 中开启)。留空则自动探测。',
    'settings.obsidianCli.placeholder': '/usr/local/bin/obsidian',
