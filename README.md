...
- 🔗 **引用当前笔记**:输入框上方工具栏的「引用」图标 —— 笔记中**选中了文字**则把选中部分以引文块插入输入框(超长自动截断,附来源行);**无选区**则插入 wikilink(对标 Claudian 的 New conversation)
- @ **提及笔记**:在输入框输入 `@` 即可搜索并引用仓库中的任意笔记 —— 按标题/路径实时匹配,选中后以 `[[路径]]` 形式插入(范围跟随设置中的「工作目录」)
...
- 🧰 **技能按钮**(工具栏 🔧,历史按钮左侧):浮动面板列出 DSH 技能(名称 + 描述 + 来源),点击把 `/技能名 ` 插进输入框;DSH 原生识别 `/名称` 斜杠调用并自动注入技能全文
- 🌐 界面 i18n(en / 中文)
